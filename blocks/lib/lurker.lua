...
local isdir = function(item)
   return love.filesystem.getInfo(item).type == "directory" end

local lastmodified = function(item)
   return love.filesystem.getInfo(item).modtime end
...
function lurker.getchanged()
   local function fn(f)
      return (f:match("%.lua$") or f:match("%.fnl$")) and
